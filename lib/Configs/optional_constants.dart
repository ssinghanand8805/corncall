...
const Color colorCallbuttons = Colors
    .blueAccent; // applied to call buttons in Video Call & Audio Call .dart pages

const bool deleteMessaqgeForEveryoneDeleteFromServer =
    true; //in group chat if delete message for everyone is tapped, It will be the message tile will be deleted from server (if this switch is true) OR if this switch is false, it will show the deleted messase as "Message is deleted"
...
const IsLazyLoadingChat = true; //## under development yet
...
const IsShowLanguageNameInNativeLanguage =
    true; // if "true", users can see the language name in respective language
...
const IsShowUserFullNameAsSavedInYourContacts =
    true; //Warning: UNDER DEVELOPMENT //If true,All the users /peer name will be show as you have saved in contact. If "false", then the name will be the one which user has saved in his profile.
...
const IsShowSearchTab =
    true; //If true, search chat tile name will be shown in homepage. it will search only personal chats name.
...
const AgoraVideoResultionWIDTH =
    1920; //Agora Video Call Resolution, see details - https://docs.agora.io/en/video-calling/develop/ensure-channel-quality?platform=web
...
const IsShowLanguageChangeButtonInHomePage = true;
//If 'true', shows change language button in settings page, and if 'false' show on homepage appbar.

const IsShowAppLogoInHomepage =
    false; //If 'true', show applogo_dark.png/applogo_dark.png in the homepage top appbar, and if 'false' shows only the AppLabel text in homepage appbar. If 'true' it is recommended to set the app logo with transparent background.

const IsShowTextLabelsInPhotoVideoEditorPage =
    true; //If 'true', text labels like 'trim', 'cover', 'rotate' etc. will be shown at PhotoEditor.dart & VideoEditor.dart

const int MaxTextlettersInStatus =
    200; // maximum characters length while creating text status Status
